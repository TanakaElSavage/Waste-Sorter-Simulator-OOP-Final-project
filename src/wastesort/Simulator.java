...
public class Simulator {
    ...
    public Simulator(long seed) { this.factory = new WasteFactory(seed); }
    public Simulator() { this.factory = new WasteFactory(); }
    ...
    private void runMenu() {
        ...
        while (running) {
            ...
            switch (choice) {
                case "1": generateAutoItems(); break;
                case "2": viewGeneratedItems(); break;
                case "3": createManualItemMenu(); break;
                case "4": sortFromAutoList(); break;
                case "5": sortFromManualList(); break;
                case "6": viewBins(); break;
                case "7": printSummary(); break;
                case "8": saveSummaryCsv(); break;
    ...
    private void generateAutoItems() {
        ...
        int n;
        try { n = Integer.parseInt(line); } catch (NumberFormatException e) { System.out.println("Invalid number."); return; }
        for (int i = 0; i < n; i++) autoGenerated.add(factory.randomWaste());
    ...
    private void viewGeneratedItems() {
        ...
        if (autoGenerated.isEmpty()) System.out.println("No items generated yet.");
        else {
    ...
    private void createManualItemMenu() {
        ...
        try {
            Waste w = null;
            switch (type) {
                case "1": w = createPlasticPrompt(); break;
                case "2": w = createMetalPrompt(); break;
                case "3": w = createOrganicPrompt(); break;
                case "4": w = createPaperPrompt(); break;
    ...
    private void sortFromAutoList() {
        if (autoGenerated.isEmpty()) { System.out.println("No auto-generated items."); return; }
        viewGeneratedItems();
        ...
        try {
            ...
            if (idx < 0 || idx >= autoGenerated.size()) { System.out.println("Index out of range."); return; }
            Waste w = autoGenerated.get(idx);
            boolean added = sorter.autoSort(w);
            if (added) { System.out.println("Sorted: " + w.getName()); autoGenerated.remove(idx); }
            else System.out.println("Could not add item to bin (maybe full).");
        } catch (NumberFormatException ex) { System.out.println("Invalid input."); }
    }

    private void sortFromManualList() {
        if (manualCreated.isEmpty()) { System.out.println("No manually created items."); return; }
        ...
        try {
            int idx = Integer.parseInt(input);
            if (idx < 0 || idx >= manualCreated.size()) { System.out.println("Index out of range."); return; }
            Waste w = manualCreated.get(idx);
            boolean added = sorter.autoSort(w);
            if (added) { System.out.println("Sorted: " + w.getName()); manualCreated.remove(idx); }
            else System.out.println("Could not add item to bin (maybe full).");
        } catch (NumberFormatException ex) { System.out.println("Invalid input."); }
    }
    ...
    private void saveSummaryCsv() {
        ...
        if (filename.isEmpty()) filename = "waste_summary.csv";
